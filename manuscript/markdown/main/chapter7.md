...
{icon=bomb}
G> `EncodedPersistentScript.ps1` now needs to be hosted somewhere. As in the above plays, we will host it on our Kali Linux VM that we have already used to:  
G> 1. Listen for the reverse shell  
G> 2. Host `powershell_hacking.bat` as `/var/www/html/payload.txt`  
G> As in the previous plays, Start your web server:  
G> `Service apache2 start`  
